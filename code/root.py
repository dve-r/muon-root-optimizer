...
class ROOT(Optimizer):
    ...
    Adds Soft-Thresholding and AdaNewton to the Muon base.
    """
    def __init__(self, params, lr=0.02, momentum=0.95, threshold=1e-3):
        # threshold is the lambda for soft-thresholding
        defaults = dict(lr=lr, momentum=momentum, threshold=threshold)
        super().__init__(params, defaults)
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                grad = p.grad
                assert grad.ndim >= 2, "ROOT strictly requires 2D+ parameters."
                ...
                # ROOT MECHANISM 1: Soft-Thresholding (Proximal Optimization)
                # Suppresses extreme outlier gradients to stabilize the momentum
                robust_buf = torch.sign(buf) * torch.clamp(torch.abs(buf) - threshold, min=0.0)
                
                original_shape = robust_buf.shape
                if robust_buf.ndim > 2:
                    G = robust_buf.view(original_shape[0], -1)
                else:
                    G = robust_buf
                
                # Speed optimization via transpose
                transposed = False
                if G.shape[0] > G.shape[1]:
                    G = G.T
                    transposed = True
                ...
                # ROOT MECHANISM 2: AdaNewton
                # Calculate shape-dependent coefficients based on aspect ratio
                dim_min, dim_max = G.shape[0], G.shape[1]
                ratio = dim_min / max(dim_max, 1)
                
                # Dynamic adjustment (Simplified heuristic based on ROOT's spectral tuning)
                # As the matrix gets more rectangular (ratio -> 0), coefficients shift slightly 
                # to maintain orthogonalization precision.
                a = 3.4445 + (0.1 * (1 - ratio))
                b = -4.7750 - (0.15 * (1 - ratio))
                c = 2.0315 + (0.05 * (1 - ratio))
                ...
                dim_out, dim_in = original_shape[0], (robust_buf.numel() // original_shape[0])
                scale = max(1, dim_out / dim_in) ** 0.5
                update = (X * scale).view(original_shape)
...
optimizer_root = ROOT(muon_params, lr=0.01, momentum=0.95, threshold=1e-3)
optimizers = [optimizer_root, optimizer_adamw]

# CosineAnnealingLR scheduler
scheduler_root = optim.lr_scheduler.CosineAnnealingLR(optimizer_root, T_max=EPOCHS)
scheduler_adamw = optim.lr_scheduler.CosineAnnealingLR(optimizer_adamw, T_max=EPOCHS)
schedulers = [scheduler_root, scheduler_adamw]
